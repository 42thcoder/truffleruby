...
class TruffleTool
  ...
  def load_gem_configuration
    if @gem_name
      if (hash = TruffleTool.config_for(@gem_name))
  ...
  class CIEnvironment
    ...
    def do_definition(name)
      definition = TruffleTool.ci_for name.to_sym
    ...
    def declare_options(parse_options: true, **parser_options)
      ...
      TruffleTool.build_option_parser(parser_options, @options, option_parser: @option_parser)
      @options.merge! TruffleTool.default_option_values(parser_options)
    ...
    def setup(*options)
      Dir.chdir(testing_dir) do
        TruffleTool.new(['setup', *options].compact,
